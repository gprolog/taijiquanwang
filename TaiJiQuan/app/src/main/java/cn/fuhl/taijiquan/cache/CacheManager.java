package cn.fuhl.taijiquan.cache;
...
import cn.fuhl.taijiquan.utils.FileUtils;
...
public class CacheManager {
                ...
                synchronized (cacheThread) {
                    while (true) {
                        if (cacheTasks != null && !cacheTasks.isEmpty()
                                && cacheTasks.size() > 0) {
                            TLog.logi(TAG, "begin cache new task :" + cacheTasks.get(0).getKey());
                            saveCache(cacheTasks.get(0));
                            if (cacheTasks != null && !cacheTasks.isEmpty())
                                cacheTasks.remove(0);
                        } else {
                            try {
                                TLog.logi(TAG, "waitting new cache task...");
                                wait();
    ...
    public static synchronized void clearNeedClearCache(int status) {
        ...
        if (needClear(TYPE_INTERNAL)) {
            TLog.logi(TAG, "The internal cache space is not enough, clear cache...");
            clearProportionCache(TYPE_INTERNAL);
        }
        if (Environment.getExternalStorageState().equals(
                Environment.MEDIA_MOUNTED)
                && needClear(TYPE_EXTERNAL)) {
            TLog.logi(TAG, "The external cache space is not enough, clear cache...");
    ...
    public static byte[] getCache(String key) {
        ...
        if (cache != null && cache.getExpire() > System.currentTimeMillis()
            ...
            try {
                ...
                TLog.logi(TAG, "get cache data: " + key);
                return data;
            } catch (IOException e) {
                e.printStackTrace();
            ...
            TLog.logi(TAG, "get cache data fail: " + key);
        }
        return null;
    }
        ...
        if (dbHelper != null) {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                if (cursor.moveToNext()) {
                    ...
                    TLog.logi(TAG, "cache cursor is no data with:" + key);
                }
            } else {
                TLog.logi(TAG, "cache cursor is empty with key:" + key);
            }
            if (cursor != null)
                cursor.close();
        } else {
            TLog.logi(TAG, "db helper is null");
        }
        TLog.logi(TAG, "get cache file :" + cache);
        return cache;
    }
    ...
    private static synchronized boolean needClear(int type) {
        if (type == TYPE_INTERNAL
            ...
            TLog.logi(TAG, "The cache space [" + type + "] need clear");
            return true;
        }
    ...
    private static File saveByteToFile(CacheTask task) {
        ...
        if (task.getKey() == null || task.getContent() == null
            ...
            TLog.logi(TAG, "set cache data: cache task is null");
            return null;
        }
        // 如果无法生成缓存文件名直接返回
        String fileName = getCacheFileName(task.getKey());
        if (fileName == null) {
            TLog.logi(TAG, "set cache data: cache name is null");
            return null;
        }
        ...
        if (task.getType() == TYPE_INTERNAL) {
            if (cacheDirInternal != null && cacheDirInternal.exists()
                ...
                TLog.logi(TAG, "set cache data: cache internal dir is not exists");
                return null;
            }
            ...
            if (cacheDirExternal != null && cacheDirExternal.exists()
                ...
                TLog.logi(TAG, "set cache data: cache external dir is not exists");
                return null;
            }
        }
        ...
        try {
            ...
            TLog.logi(TAG, "set cache data: move cache file exception " + e);
            e.printStackTrace();
        }
        return cacheFile;
    }
    ...
    public static void setCache(String key, byte[] content, long expire,
                                int type) {
        TLog.logi(TAG, "add cache task:" + key + " expire:" + expire);
